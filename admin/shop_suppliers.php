...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link', array('href' => 'shop_suppliers.php?act=add&shop_id='.$shop_id, 'text' => $_LANG['add_suppliers']));
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('shop_suppliers_list.htm');
}
...
elseif (in_array($_REQUEST['act'], array('add', 'edit')))
{
    ...
    if ($_REQUEST['act'] == 'add')
    {
        $sql = "SELECT * FROM " . $ecs->table('shop') . " WHERE shop_id = '$shop_id'";
        $shop = $db->getRow($sql);
        if (count($shop) <= 0)
        {
            sys_msg('shop does not exist');
        }
        $suppliers = array();
        $suppliers['shop_id'] = $shop['shop_id'];
        $suppliers['pcat_id'] = $shop['pcat_id'];
        $suppliers['cat_id']  = $shop['cat_id'];        
        ...
        $smarty->assign('action_link', array('href' => 'shop_suppliers.php?act=list&shop_id='.$shop_id, 'text' => $_LANG['suppliers_list']));
        ...
        $smarty->display('shop_suppliers_info.htm');
...
elseif (in_array($_REQUEST['act'], array('insert', 'update')))
{
    ...
    if ($_REQUEST['act'] == 'insert')
    {
        ...
        $suppliers['shop_id']           = !empty($_POST['shop_id'])              ? intval($_POST['shop_id'])        : 0;
        $suppliers['pcat_id']           = !empty($_POST['pcat_id'])             ? intval($_POST['pcat_id'])         : 0;
        $suppliers['cat_id']            = !empty($_POST['cat_id'])              ? intval($_POST['cat_id'])          : 0;
        $suppliers['agency_id']         = !empty($_POST['agency_id'])           ? intval($_POST['agency_id'])       : 0;   // 管理机构预留
        ...
        /* 提示信息 */
        $links = array(array('href' => 'shop_suppliers.php?act=add&shop_id='. $suppliers['shop_id'],  'text' => $_LANG['continue_add_suppliers']),
                       array('href' => 'shop_suppliers.php?act=list&shop_id='. $suppliers['shop_id'], 'text' => $_LANG['back_suppliers_list'])
                       );
