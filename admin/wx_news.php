...
$exc = new exchange($ecs->table("wx_news"), $db, 'news_id', 'news_name');
...
if ($_REQUEST['act'] == 'list'){
    ...
    assign_query_info();
    $smarty->assign('ur_here',     $_LANG['051_wx_news_list']);
    $smarty->assign('action_link', array('text' => $_LANG['051_wx_news_add'], 'href' => 'wx_news.php?act=add'));
    ...
    $smarty->display('wx_news_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('wx_article_list.htm'), '', array('filter' => $cards_list['filter'], 'page_count' => $cards_list['page_count']));
}
/*------------------------------------------------------ */
//-- 删除图文资源
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('ur_here',     $_LANG['051_wx_news_add']);
    $smarty->assign('action_link', array('text' => $_LANG['051_wx_news_list'], 'href' => 'wx_news.php?act=list'));
    $smarty->assign('form_action', 'insert');

    assign_query_info();
    $smarty->display('wx_news_info.htm');
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if (!$is_only)
    {
        sys_msg(sprintf($_LANG['news_name_exist'], stripslashes($_POST['news_name'])), 1);
    }
    ...
    $link[0]['href'] = 'wx_news.php?act=add';

    $link[1]['text'] = $_LANG['back_list'];
    $link[1]['href'] = 'wx_news.php?act=list';

    sys_msg($_POST['news_name'].$_LANG['news_add_succeed'],0, $link);
}
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT news_id, news_name, news_desc FROM " .$ecs->table('wx_news'). " WHERE news_id='$_REQUEST[id]'";
    $news = $db->GetRow($sql);

    /* 查询 */
    $sql = "SELECT news_id, article_id, wx_media_id, title, author, img_url, source_url, content, digest, show_cover_pic".
           " FROM ".$GLOBALS['ecs']->table('wx_article').
    ...
    $smarty->assign('ur_here',     $_LANG['news_edit']);
    $smarty->assign('action_link', array('text' => $_LANG['051_wx_news_list'], 'href' => 'wx_news.php?act=list&' . list_link_postfix()));
    $smarty->assign('news',         $news);
    $smarty->assign('article_list', $article_list);    
...
//-- ajax编辑卡片名字
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'edit_news_name')
{
    check_authz_json('wx_manage');
    $card_id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
...
function news_list()
{
    ...
    if ($result === false)
    {
        $filter['sort_by']    = empty($_REQUEST['sort_by']) ? 'news_id' : trim($_REQUEST['sort_by']);
        $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);

        /* 分页大小 */
        $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('wx_news');
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        $filter = page_and_size($filter);

        /* 查询 */
        $sql = "SELECT news_id, news_name, add_time, news_desc".
               " FROM ".$GLOBALS['ecs']->table('wx_news').
    ...
    $arr = array('news_list' => $card_list, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
